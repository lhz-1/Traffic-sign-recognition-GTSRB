...
def loss(X_data, y_data):               #loss函数 softmax
...
def evaluate(X_data, y_data):              # accuracy
...
with tf.Session() as sess:
    ...
    saver.save(sess, 'D:/projects/Traffic-Sign-Classify/model/' + model_name)
    print("Model saved")
    ...
    y1 = train_losss
    y2 = train_accuracys
    plt.plot(a, y1, marker='o', label= 'loss')
    plt.plot(a, y2, marker='*', label='accuracy')
    plt.title('loss and accuracy')
    plt.xlabel('x')
    plt.ylabel('y')
    plt.legend()
    my_x_ticks = np.arange(0, EPOCHS, 5)
